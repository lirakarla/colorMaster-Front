...
import {MoreOutlined, DeleteOutlined, EditOutlined, HeartOutlined,HeartFilled } from "@ant-design/icons";
...
const content=(idPaleta, onDelete, nombre)=>(
    <div>
        ...
        <a onClick={()=> onDelete(nombre, idPaleta)} className="paletaCard-opcion paletaCard-eliminar" >
        <span>
            Eliminar
       </span>
...
const PaletaCard= ({onDelete, paleta,mostrarCorazon,onFavoritoUpdate}) => {
  ...
  const updateFavorito=()=>{
    axios.put("http://localhost:4000/paleta/favorito",{
        idUsuario:2,idPaleta:paleta.idPaleta,favorito:!paleta.favorito
    }).then(()=>{
  ...
  const renderIcon=()=>{
    if(mostrarCorazon){
        if(paleta.favorito){
            return(
                <HeartFilled onClick={updateFavorito} style={{color:"red", float: "right",marginRight: "10px",marginTop: "5px"}}/>
            )
        }
        else{
            return(
                <HeartOutlined onClick={updateFavorito} style={{color:"white", float: "right",marginRight: "10px",marginTop: "5px"}}/>
            )
        }
    }
    else{
    ...
    <div className="paletaCard">
        ...
        </div>
        {
            paleta.colores.map(color=>{
                return(
                    <div onClick={()=>handleColorClick(color.color)} className="paletaCard-colores" style={{backgroundColor:color.color}}/>
                )
            })
        }
        ...
        {paleta.categorias.slice(0,3).map(categoria=> {
            return(
                <span className="paletaCard-categoria">
                     {categoria.nombre}
                </span>
            )
        })}
        {
            paleta.categorias.length>3 && 
                    <Tooltip title={paleta.categorias.map(categoria=>categoria.nombre).slice(3,paleta.categorias.length).join(", ")}>
                        <span>...</span>
                     </Tooltip>
